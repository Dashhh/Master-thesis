...
For a given graph $G$, we say that $S$ is a \ssf{} if $S$ spans $G$ and every connected component of $S$ is a star. 
...
Now we need to check if the obtained graph is a spanning star forest. There is one non-trivial operation that the algorithm does. Specifically, if the root has not  been added during the for loop, we connect the root to any existing star without checking whether it remains a correct star.

\begin{claim}\label{ssf root}
	If $(u_1,u_2),(u_2,u_3) \in E(S)$, $u_2 = parent(u_1)$ and $u_3=parent(u_2)$, then $u_3$ is the root and $u_3$ is incident to one edge of the solution only.
\end{claim}
...
\begin{proof}
	Clearly, $S$ is a forest. So, it suffices to check that there are no isolated vertices and the solution does not contain a path of length $3$. The first claim is true because we enumerate over all vertices and pair them up with its parent if a vertex has not been added to the solution in previous iteration. So is the second claim based on the information inferred from Proposition \ref{ssf root}.
\end{proof}

Having proven the correctness of Algorithm \ref{alg1} we proceed to the complexity analysis i.e. we prove the Theorem \ref{ssfp-thm}.

\begin{proof}[Proof (of Theorem \ref{ssfp-thm})]
...
The problem stated without any constraints is simple. Both decision and constructive variant of the problem can be solved in linear time. The next variations investigated in this paper yield more complex results.
...
\begin{proof}
	...
	\begin{enumerate}
		...
		\begin{equation*}
			...
			\end{cases}
		\end{equation*}
		is a valid domination mapping that satisfies $\Ima \mu \subsetneq \Ima \mu'$. Contradiction, we assumed that $\mu$ is a dominating mapping that maximizes $|\imath \mu|$.
	\end{enumerate}
...
\begin{theorem}\label{dom-ssf reduction}
	There is a parameterized reduction that takes an instance $(G,k)$ of \domstep{} and returns instance $(G',k)$ of \mssfp{} such that it doubles the number of vertices pessimistically.
\end{theorem}
